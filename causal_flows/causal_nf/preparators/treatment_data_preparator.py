...
from ..datasets.treatment_data import TreatmentDataset
from ..preparators.tabular_preparator import TabularPreparator
from ..sem_equations import sem_dict
from ..utils.io import dict_to_cn

from ..utils.scalers import StandardTransform
...
class TreatmentDataPreparator(TabularPreparator):
    def __init__(self, add_noise, name, num_sensitive,
                 num_covariate, num_treatment, binary_dims, **kwargs):
        ...
        sem_fn = sem_dict["treatment_data"](sem_name="dummy", num_sensitive=self.num_sensitive,
                                            num_treatment=self.num_treatment, num_covariate=self.num_covariate)

        self.adjacency = sem_fn.adjacency

        self.num_nodes = len(sem_fn.functions)

        # self.binary_dims = tuple([i for i in range(self.num_nodes) if i not in self.numeric_dims])
        self.binary_dims = binary_dims
        self.numeric_dims = tuple([i for i in range(self.num_nodes) if i not in self.binary_dims])

        self.intervention_index_list = sem_fn.intervention_index_list()
        # super().__init__(name=f"treatment_{self.treat_data_name}", task="modeling", **kwargs)
        super().__init__(name=f"{self.name}", task="modeling", **kwargs)
    ...
    def params(cls, dataset):
        ...
        my_dict = {
            "add_noise": dataset.add_noise,
            # AM: Need to add these on my own much like add_noise.
            # "treat_data_name": dataset.treat_data_name,
            "name": dataset.name,
            "num_sensitive": dataset.num_sensitive,
            "num_covariate": dataset.num_covariate,
            "num_treatment": dataset.num_treatment,
            # "numeric_dims": dataset.numerical_dims
            "binary_dims": dataset.categorical_dims
        }
    ...
    def loader(cls, dataset):
        my_dict = TreatmentDataPreparator.params(dataset)
    ...
    def compute_ate(self, index, a, b, num_samples=10000):
        ...
        ate = torch.rand((self.num_nodes - 1)) * 2 - 1.0
        return ate
    ...
    def _split_dataset(self, dataset_raw):
        ...
        for i, split_s in enumerate(self.split):
            dataset = TreatmentDataset(
                ...
                name=self.name, num_sensitive=self.num_sensitive,
                num_covariate=self.num_covariate, num_treatment=self.num_treatment,
