...
class TreatmentDataset(Dataset):
    def __init__(self, root_dir: str, split: str, seed: int = None,
                 name: str = None, num_sensitive: int = None,
                 num_covariate: int = None, num_treatment: int = None,
                 binary_dims: tuple = None,):
        ...
        # AM: Adding in. I think these will be needed as input to become flexible.
        assert num_sensitive is not None and num_covariate is not None and num_treatment is not None
        self.num_sensitive = num_sensitive
        self.num_covariate = num_covariate
        self.num_treatment = num_treatment
        assert num_sensitive + num_covariate + num_treatment > 0, "Number of features is zero!"
        self.num_decision = 1
        ...
        # self.binary_dims = list(range(21))
    ...
    def _create_data(self):
        ...
        df = pd.read_csv(os.path.join(self.root_dir, f"{self.name}", "data.csv"), header=0)
    ...
    def __str__(self):
        my_str = f"Dataset Treatment {self.name}\n"
