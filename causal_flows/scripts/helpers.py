...
from ..causal_nf.utils import wandb_local as wandb_local
...
def update_names(df, column_mapping=None, model="flow"):
    ...
    mapping = {}
    # print(df_tmp.dataset__name, df_tmp.dataset__sem_name)
    for dataset_name in df_tmp.dataset__name.unique():
        ...
        mapping[f"{dataset_name}"] = f"{part_1}"
        # for sem_name in df_tmp.dataset__sem_name.unique():
        #     part_1 = f"{dataset_name.upper()}"
        #     if math.isnan(sem_name):
        #         part_2 = "dummy"
        #     else:
